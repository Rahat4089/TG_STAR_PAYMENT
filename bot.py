...
# Import verification-related settings and functions
from config import ADMINS, START_MSG, IS_VERIFY, VERIFY_EXPIRE, SHORTLINK_URL, SHORTLINK_API, TUT_VID
...
async def on_donate_handler(client, message: Message):
    ...
    text = message.text
    # Check if user is admin (admins do not need verification)
...
@bot.on_message(filters.regex(r'^/start verify_') & filters.private)
